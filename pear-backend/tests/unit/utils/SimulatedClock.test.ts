...
describe("SimulatedClock", () => {
  ...
  describe("advanceDay", () => {
    ...
    it("should throw error if clock not initialized", () => {
      // Manually reset internal state before testing
      // @ts-expect-error - accessing private fields for test purposes
      SimulatedClock.simulatedStartDate = null;
      // @ts-expect-error
      SimulatedClock.currentSimulatedDayOffset = 0;

      expect(() => SimulatedClock.advanceDay()).toThrow(
        "SimulatedClock not initialized. Call setSimulationStartTime first."
      );
    });
  });
